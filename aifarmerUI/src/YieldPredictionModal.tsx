...
const YieldPredictionModal: React.FC<YieldPredictionModalProps> = ({ show, onClose, crops, categories }) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h2 style={{ color: "#16a34a", fontWeight: 700, fontSize: 24, margin: 0 }}>📊 Yield Prediction</h2>
              ...
              {filteredCrops.map((crop, idx) => (
                ...
                >
                  ...
                  <div style={{background: '#f0fdf4', borderRadius: 8, padding: '10px 14px', marginBottom: 10}}>
                    <span style={{fontWeight: 600, color: '#15803d'}}>📦 Yield Prediction:</span>
                    <ul style={{margin: '8px 0 0 18px', color: '#166534', fontSize: 15}}>
                      <li><strong>Expected Yield:</strong> {crop.economicsPerAcre.yield}</li>
                  ...
                  {/* Direct link if available (for demo, link to Google search for crop yield prediction) */}
                  <a
                    href={`https://www.google.com/search?q=${encodeURIComponent(crop.name + ' yield prediction')}`}
                    target="_blank"
